...
def plot_requests_per_second(df: DataFrame, thresholds: list) -> None:
    ...
    plt.title('Number of Requests per Second in a Flash Crowd', fontsize=20, weight='bold')
...
def main():
    ...
    df = pd.read_csv(os.path.join(current_directory, 'flash_crowd.csv'))
    max_requests = get_max_requests_per_second(df)

    # Calculate when flash crowd is detected
    timestamp = find_timestamp_above_threshold(df, 'date', detection_threshold)

    # Plot Flash Crowd attack
    plot_requests_per_second(df,[detection_threshold,denial_of_service_threshold])
    plt.savefig(os.path.join(current_directory, 'flash_crowd.png'))
